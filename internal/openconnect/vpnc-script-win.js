...
function ecnuApplySplitRoutes() {
    var routes = env("ECNU_SPLIT_ROUTES") || ecnuDefaultSplitRoutes;
    var items = routes.split(/[ ,;]+/);
    var count = 0;
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        var cidr = item.split("/");
        if (cidr.length != 2 || !ecnuValidIPv4(cidr[0]) || !cidr[1].match(/^[0-9]+$/))
            continue;
        ...
        env("CISCO_SPLIT_INC_" + count + "_ADDR") = cidr[0];
        env("CISCO_SPLIT_INC_" + count + "_MASK") = ecnuMaskFromBits(bits);
        env("CISCO_SPLIT_INC_" + count + "_MASKLEN") = String(bits);
        count++;
    }

    if (count == 0) {
        env("CISCO_SPLIT_INC_0_ADDR") = "172.0.0.0";
        env("CISCO_SPLIT_INC_0_MASK") = "255.0.0.0";
        env("CISCO_SPLIT_INC_0_MASKLEN") = "8";
        count = 1;
    }

    env("CISCO_SPLIT_INC") = String(count);
    env("CISCO_SPLIT_EXC") = "";
}

ecnuApplySplitRoutes();
